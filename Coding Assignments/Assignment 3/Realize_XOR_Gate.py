...
last_fitness = 0  # Holds the fitness value of the previous generation.

data_inputs = numpy.array([[1, 1], [1, 0], [0, 1], [0, 0]])  # Array of the inputs.
data_outputs = numpy.array([0, 1, 1, 0])                     # Array of the outputs.
...
predictions = pygad.nn.predict(
    last_layer=GANN_instance.population_networks[solution_idx], data_inputs=data_inputs
)
print(
    "Predictions of the trained network : {predictions}".format(predictions=predictions)
)
...
print(
    "Number of correct classifications : {num_correct}.".format(num_correct=num_correct)
)
